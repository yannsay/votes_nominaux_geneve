...
RSGE_VOTING_CSV = os.path.join(os.path.dirname(__file__), 'fixtures', 'clean_rsge_voting.csv')
OTH_VOTING_CSV = os.path.join(os.path.dirname(__file__), 'fixtures', 'clean_oth_voting.csv')
    ...
    def test_filter_oth_votings(self):
        ...
        self.assertEqual(filter_oth_voting(voting_table=self.clean_oth_voting,
                  ...
                  selected_titres = [],
                         ...
                         (319, 41))
        self.assertEqual(filter_oth_voting(voting_table=self.clean_oth_voting,
                   selected_type_votes = ["Initiative populaire cantonale"],
                  selected_titres = [],
                                       selected_dates=self.initial_dates).shape,
                         (22, 41))
        self.assertEqual(filter_oth_voting(voting_table=self.clean_oth_voting,
                   selected_type_votes = [],
                  selected_titres = ["Initiative populaire cantonale 198 « Pour une contraception gratuite »"],
                                       selected_dates=self.initial_dates).shape,
                         (2, 41))
        self.assertEqual(filter_oth_voting(voting_table=self.clean_oth_voting,
                   selected_type_votes = ["Initiative populaire cantonale"],
                  selected_titres = ["Initiative populaire cantonale 191 « Pour une transition rapide vers le solaire à Genève »"],
                                       selected_dates=self.initial_dates
                                       ).shape,
                         (2, 41))
        self.assertEqual(filter_oth_voting(voting_table=self.clean_oth_voting,
