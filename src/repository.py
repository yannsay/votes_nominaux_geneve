...
class AppDatabase:
    ...
    def set_clean_votings(self, votings_file: str, rsge_data: pd.DataFrame) -> None:
        ...
        clean_voting['Référence'] = clean_voting['voting_affair_title_fr'].str.extract(
            pattern)
        clean_voting['Référence'] = clean_voting['Référence'].str.replace(
            '\xa0', ' ', regex=False)
        ...
        oth_voting = clean_voting.loc[pd.isna(
            clean_voting['Référence']),].reset_index()
        ...
        clean_voting = clean_voting.merge(
            clean_rsge, on="Référence", how="left")
